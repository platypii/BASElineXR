...
public class WindVelocityChart extends View {
    ...
    private void init() {
        ...
        windLabelPaint.setTextSize(20f);
    ...
    private void drawWindVectors(Canvas canvas) {
        ...
        if (gpsCircleFit != null && gpsCircleFit.pointCount >= 3) {
            ...
            String gpsLabel = WindDisplayFormatter.INSTANCE.formatWindEstimationShort(gpsWind, "GPS Wind");
            ...
            float textWidth = gpsLabelPaint.measureText(gpsLabel);
            float labelX = circleX - textWidth; // Text ends at vector tip
            float labelY = circleY;
            
            // Check for collisions and stack if needed
            for (Float existingY : windLabelPositions) {
                if (Math.abs(labelY - existingY) < 25f) { // Labels too close
                    labelY = existingY - 25f; // Stack above
                }
            }
            windLabelPositions.add(labelY);
            
            canvas.drawText(gpsLabel, labelX, labelY, gpsLabelPaint);
        }
        ...
        if (sustainedCircleFit != null && sustainedCircleFit.pointCount >= 3) {
            ...
            // Create wind estimation for formatting
            WindEstimation sustainedWind = new WindEstimation(
                sustainedCircleFit.getWindE(), sustainedCircleFit.getWindN(),
                sustainedCircleFit.rSquared, sustainedCircleFit.pointCount
            );
            String sustainedLabel = WindDisplayFormatter.INSTANCE.formatWindEstimationShort(sustainedWind, "Sustained Wind");
            ...
            // Position label so it ends at the vector end (shift text box to left)
            float textWidth = sustainedLabelPaint.measureText(sustainedLabel);
            float labelX = circleX - textWidth; // Text ends at vector tip
            float labelY = circleY;
            ...
            canvas.drawText(sustainedLabel, labelX, labelY, sustainedLabelPaint);
        }
    }
    ...
    private void drawAircraftVectors(Canvas canvas) {
        ...
        // Add groundspeed label at the end of the vector, offset to the right side
        String groundspeedLabel = String.format("Groundspeed: %.0f MPH @ %.0f°", groundspeedMph, groundTrack);
        ...
        float groundspeedLabelY = gpsVelY;
        aircraftLabelPositions.add(groundspeedLabelY);
        canvas.drawText(groundspeedLabel, groundspeedLabelX, groundspeedLabelY, groundspeedLabelPaint);
        ...
        if (gpsCircleFit != null && gpsCircleFit.pointCount >= 3) {
            ...
            // Add airspeed label at the end of the vector, offset to the right side
            String airspeedLabel = String.format("Airspeed: %.0f MPH @ %.0f°", airspeedMph, airspeedDirection);
            ...
            float airspeedLabelY = gpsVelY;
            
            // Check for collisions and stack if needed
            for (Float existingY : aircraftLabelPositions) {
                if (Math.abs(airspeedLabelY - existingY) < 25f) { // Labels too close
                    airspeedLabelY = existingY + 25f; // Stack below
                }
            }
            aircraftLabelPositions.add(airspeedLabelY);
            
            canvas.drawText(airspeedLabel, airspeedLabelX, airspeedLabelY, airspeedLabelPaint);
        }
    }
