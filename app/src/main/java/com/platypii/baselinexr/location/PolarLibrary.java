...
public class PolarLibrary {
    ...
    // Custom polars - placeholder values for user customization
    public static final WingsuitPolar CUSTOM_WINGSUIT_POLAR = new WingsuitPolar(
            "custom_wingsuit",      // polarId
            "",                     // userId
            "Wingsuit",             // type
            "Custom Wingsuit",      // name
            false,                  // isPublic
            0.16,                   // polarSlope (placeholder)
            0.50,                   // polarClo (placeholder)
            0.30,                   // polarMinDrag (placeholder)
            0.45,                   // rangeMinCl (placeholder)
            1.35,                   // rangeMaxCl (placeholder)
            A6_AOA_INDEXES,         // aoaIndexes (use A6 as template)
            A6_AOAS,                // aoas (use A6 as template)
            A6_CP,                  // cp (use A6 as template)
            A6_STALL_POINT,         // stallPoint (use A6 as template)
            3.5,                    // s (wing area in m²)
            90.0                    // m (mass in kg)
    );

    public static final WingsuitPolar CUSTOM_CANOPY_POLAR = new WingsuitPolar(
            "custom_canopy",        // polarId
            "",                     // userId
            "Canopy",               // type
            "Custom Canopy",        // name
            false,                  // isPublic
            ...
            0.45,                   // rangeMinCl (placeholder)
            1.35,                   // rangeMaxCl (placeholder)
            IBEX_UL_AOA_INDEXES,    // aoaIndexes (use Ibex as template)
            IBEX_UL_AOAS,           // aoas (use Ibex as template)
            IBEX_UL_CP,             // cp (use Ibex as template)
            IBEX_UL_STALL_POINT,    // stallPoint (use Ibex as template)
            20.0,                   // s (wing area in m²)
            100.0                   // m (mass in kg)
    );

    public static final WingsuitPolar CUSTOM_AIRPLANE_POLAR = new WingsuitPolar(
            "custom_airplane",      // polarId
            "",                     // userId
            "Airplane",             // type
            "Custom Airplane",      // name
            false,                  // isPublic
            0.484813091400691,      // polarSlope (placeholder)
            0.21896316379267053,    // polarClo (placeholder)
            0.029467123091668542,   // polarMinDrag (placeholder)
            0.21741522340551012,    // rangeMinCl (placeholder)
            0.5258731795206912,     // rangeMaxCl (placeholder)
            CARAVAN_AOA_INDEXES,    // aoaIndexes (use Caravan as template)
            CARAVAN_AOAS,           // aoas (use Caravan as template)
            CARAVAN_CP,             // cp (use Caravan as template)
            CARAVAN_STALL_POINT,    // stallPoint (use Caravan as template)
            2.0,                    // s (wing area in m²)
            77.5                    // m (mass in kg)
    );
    ...
    /**
     * Get all available polars grouped by type
     */
    public static java.util.List<WingsuitPolar> getAllPolars() {
        return java.util.Arrays.asList(
                A6_POLAR,
                AURA_FIVE_POLAR,
                BASELINE_CRACE_POLAR,
                CR_PLUS_POLAR,
                SLICK_POLAR,
                SAUSAGE_POLAR,
                CUSTOM_WINGSUIT_POLAR,
                IBEX_UL_POLAR,
                CUSTOM_CANOPY_POLAR,
                CARAVAN_POLAR,
                CUSTOM_AIRPLANE_POLAR
        );
    }

    /**
     * Get polars filtered by type
     */
    public static java.util.List<WingsuitPolar> getPolarsByType(String type) {
        java.util.List<WingsuitPolar> result = new java.util.ArrayList<>();
        for (WingsuitPolar polar : getAllPolars()) {
            if (polar.type.equalsIgnoreCase(type)) {
                result.add(polar);
            }
        }
        // Sort by name
        java.util.Collections.sort(result, (a, b) -> a.name.compareTo(b.name));
        return result;
    }
}