...
## BASElineXR Implementation Status

> **Note:** This section documents what BASElineXR currently implements from the FlySight 2 BLE interface.

### Implemented Features ✓

| Feature | Status | Notes |
|---------|--------|-------|
| **Sensor_Data Service** | ✓ Full | All 7 characteristics: GNSS, Accel, Gyro, Mag, Baro, Hum, Control Point |
| **SD_Control_Point** | ✓ Partial | SET/GET_GNSS_BLE_MASK, SET/GET_BLE_DIVIDER implemented. Fusion commands defined but untested. |
| **Device_State Service** | ✓ Full | DS_Mode subscription, DS_Control_Point (GET_FW_VERSION, GET_DEVICE_ID, SET_MODE, REBOOT) |
| **Heartbeat/Ping** | ✓ Full | Sends `0xfe` to File Transfer RX every 14.5s to prevent 30s timeout |
| **Mode Tracking** | ✓ Full | Subscribes to DS_Mode, tracks SLEEP/ACTIVE/etc. |
| **Manufacturer ID Filter** | ✓ Full | Filters by 0x09DB and checks pairing status byte |

### Not Implemented ✗

| Feature | Status | Notes |
|---------|--------|-------|
| **File_Transfer Service** | ✗ | Only heartbeat uses Service 0. No file read/write/list/delete. |
| **Starter_Pistol Service** | ✗ | UUIDs defined but not used. |
| **Go-Back-N ARQ** | ✗ | Not needed without file transfer. |
| **Battery Service** | ✗ | Not subscribed to. |
| **Device Information Service** | ✗ | Not queried. |
| **BLE Pairing Trigger** | ✗ | BASElineXR relies on Quest system-level Bluetooth pairing (double-tap FlySight button). |

### Implementation Details

**Connection Flow:**
1. Scan for devices named "FlySight" with manufacturer ID 0x09DB
2. Check pairing status byte (skip devices in pairing mode 0x01)
3. Connect and request MTU 256
4. Start heartbeat on Service 0 / Char RX (`0xfe` every 14.5s)
5. Subscribe to SD_Control_Point (Service 1 / Char 0x06)
6. Subscribe to DS_Mode (Service 3 / Char 0x05)
7. On control point ready or ACTIVE mode transition: subscribe to all sensor characteristics

**Sensor Data Parsing:**
- All sensor parsers handle the variable-length mask format correctly
- ImuCorrelator pairs accel/gyro packets by timestamp for combined MImuData output
- Time synchronization via MTimeSync (GPS time → sensor time mapping)
...
> **BASElineXR Note:** BASElineXR does NOT implement the "read secure characteristic to trigger pairing" flow. Instead, pairing is handled at the Quest system level (Settings > Devices > Bluetooth). User double-taps FlySight button to enter Pairing Request Mode, then pairs via Quest system UI. BASElineXR only connects to already-bonded devices.
...
6.  **Connection Timeout & Ping:**
    ...
    > **BASElineXR Implementation:** Heartbeat is sent via `startHeartbeat()` in `Flysight2Protocol.java`. Writes `0xfe` to `flysightService0` / `flysightCharacteristicRX` every 14.5 seconds. This runs continuously in both SLEEP and ACTIVE modes to ensure the connection stays alive.
...
> **BASElineXR Note:** File transfer operations are NOT implemented. BASElineXR only uses Service 0 for the heartbeat/ping mechanism to keep the connection alive.
...
> **BASElineXR Note:** Fully implemented. All 7 sensor characteristics are subscribed. Accel/Gyro packets are correlated by timestamp via `ImuCorrelator` to produce combined `MImuData` output. Divider configuration available via `Flysight2ControlPoint.setBleDivider()`.
...
> **BASElineXR Note:** Starter Pistol service is NOT implemented. The service UUID is defined but characteristics are never used.
...
> **BASElineXR Note:** Fully implemented. Subscribes to DS_Mode for mode change notifications. DS_Control_Point commands available via `Flysight2ControlPoint` (getFirmwareVersion, getDeviceId, setMode, rebootDevice).
...
> **BASElineXR Note:** Standard BLE services are NOT subscribed to. Battery level and device info could be added in future.
